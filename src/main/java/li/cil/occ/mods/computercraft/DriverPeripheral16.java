...
public final class DriverPeripheral16 extends DriverPeripheral<IPeripheral> {
	...
	static {
		...
		try {
			getPeripheralAt = Class.forName(
					"dan200.computercraft.ComputerCraft").getMethod(
					"getPeripheralAt", World.class, int.class, int.class,
					int.class, int.class);
		} catch (Exception e) {
			if (Loader.instance().getIndexedModList().get("ComputerCraft")
					.getVersion().startsWith("1.6")) {
				OpenComponents.Log
						.log(Level.WARNING,
								"Error getting access to ComputerCraft peripherals.",
								e);
			}
		}
	...
	protected IPeripheral findPeripheral(World world, int x, int y, int z) {
		if (ComputerCraft_getPeripheralAt != null) {
			try {
				return (IPeripheral) ComputerCraft_getPeripheralAt.invoke(null,
						world, x, y, z, -1);
			} catch (Exception e) {
				OpenComponents.Log
						.log(Level.WARNING,
								String.format(
										"Error accessing ComputerCraft peripheral @ (%d, %d, %d).",
										x, y, z), e);
			}
		}
		return null;
	...
	@Override
	public ManagedEnvironment createEnvironment(final World world, final int x,
			final int y, final int z) {
		return new Environment(findPeripheral(world, x, y, z));
	}

	public static class Environment extends
			li.cil.oc.api.prefab.ManagedEnvironment implements
		...
		public Object[] invoke(final String method, final Context context,
			...
			if (accesses.containsKey(context.node().address())) {
				...
				// The calling contexts is not visible to us, meaning we never
				// got
		...
		public void onConnect(final Node node) {
			...
			if (node.host() instanceof Context) {
				final FakeComputerAccess access = new FakeComputerAccess(this,
						(Context) node.host());
		...
		public void onDisconnect(final Node node) {
			...
			if (node.host() instanceof Context) {
				final FakeComputerAccess access = accesses.remove(node
						.address());
				if (access != null) {
		...
		private static class FakeComputerAccess implements IComputerAccess {
			...
			public FakeComputerAccess(final Environment owner,
			...
			public void close() {
				for (li.cil.oc.api.network.ManagedEnvironment fileSystem : fileSystems
						.values()) {
			...
			public String mount(final String desiredLocation, final IMount mount) {
				...
				return mount(desiredLocation,
						FileSystem.asManagedEnvironment(FileSystem
								.fromComputerCraft(mount)));
			}

			@Override
			public String mountWritable(final String desiredLocation,
					final IWritableMount mount) {
				if (fileSystems.containsKey(desiredLocation)) {
					return null;
				}
				return mount(desiredLocation,
			...
			private String mount(final String path,
					final li.cil.oc.api.network.ManagedEnvironment fileSystem) {
				fileSystems.put(path, fileSystem); // TODO This is per
			...
			public void unmount(final String location) {
				final li.cil.oc.api.network.ManagedEnvironment fileSystem = fileSystems
						.remove(location);
				if (fileSystem != null) {
					fileSystem.node().remove();
				}
		...
		/**
		 * Since we abstract away anything language specific, we cannot support
		 * the Lua context specific operations ComputerCraft provides.
		 */
